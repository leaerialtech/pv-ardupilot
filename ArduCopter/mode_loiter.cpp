...
void ModeLoiter::run()
{
    static bool was_ignoring = false;
    const bool ignoring_sticks = (copter.failsafe.radio || this->isBrakeMode); 
    ...
    // on entry to ignore: re-anchor targets to current state
    if (ignoring_sticks && !was_ignoring) {
        loiter_nav->init_target();                 // anchor XY to here/now
        attitude_control->reset_yaw_target_and_rate(); 
    }
    was_ignoring = ignoring_sticks;
    ...
    if (!ignoring_sticks) {
        ...
        // behave exactly like perfectly centered pilot sticks
