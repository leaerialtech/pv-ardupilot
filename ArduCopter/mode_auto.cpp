...
bool ModeAuto::verify_command(const AP_Mission::Mission_Command& cmd)
{
	...
	//Basically COMMANDS ARE complete when the stock function verify_nav_wpt finishes 
	//Right now we hacked in some code such that MAV_CMD_NAV_WAYPOINT waits for a signal 
	//that all mission_items completed. 
...
void ModeAuto::wp_run()
{
    ...
    static int proxCnt = 0;
  	//earlier versions had this defined, i'm just hard-coding this for now 
    if(avoid && oapath && oapath->getType() == 10) // oapath->OA_PATHPLAN_PRECISIONVISION_STOP 
    {
                ...
                if (proximity->get_horizontal_distances(dist_array)) {
                    ...
                    for (uint8_t i = 0; i < PROXIMITY_MAX_DIRECTION; i++) 
                    {
                        ...
                        if( dist > 0 && margin > 0 && dist < margin){
                            if(copter.get_mode() != (uint8_t)Mode::Number::BRAKE){
                                proxFound = true; 
                                if(proxCnt++ >= 10){
