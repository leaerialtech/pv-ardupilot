...
bool ModeAuto::verify_command(const AP_Mission::Mission_Command& cmd)
{
    ...
    case MAV_CMD_NAV_TAKEOFF:
        ...
        mission_item_reached = cmd_complete;
        break;

    case MAV_CMD_NAV_WAYPOINT:
        cmd_complete = verify_nav_wp(cmd);
        if(cmd_complete){
            //special check for nav_waypoint as we don't want to send mission item reached unless we actually hit there,
            //but we do want to consider if it is "completed" (e.g skipped/fast-waypointed) 
            mission_item_reached = copter.wp_nav->reached_wp_destination();
        }
        break;
    ...
    // send message to GCS
    if (cmd_complete && mission_item_reached) {
        gcs().send_mission_item_reached_message(cmd.index);
        printf("reached mission item num %d\n", cmd.index);
    }
...
void ModeAuto::wp_run()
{
    ...
    if(avoid && oapath && oapath->getType() == oapath->OA_PATHPLAN_PRECISIONVISION_STOP)
    {
        AP_Proximity *proximity = AP::proximity();
        if(proximity && proximity->get_status() == AP_Proximity::Status::Good) {
            
            AP_Proximity::Proximity_Distance_Array dist_array;
                if (proximity->get_horizontal_distances(dist_array)) {
                    for (uint8_t i = 0; i < PROXIMITY_MAX_DIRECTION; i++) {
                    float dist = dist_array.distance[i];
...
void ModeAuto::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    ...
    //special value which I have hacked to make
    //flight controller not advance until all do_cmds have finished. 
    bool isSpecialCase = false; 
    if(loiter_time_max == 999)
    {
        isSpecialCase = true; 
        loiter_time_max = 1;
        
    }
    copter.wp_nav->set_execute_do_cmds_before_next_nav(isSpecialCase);
 // Set wp navigation target
        ...
        if (!isSpecialCase &&  loiter_time_max == 0 && mission.get_next_nav_cmd(cmd.index+1, temp_cmd)) {
...
{
    if(copter.wp_nav->waypoint_completed()){
        return true; 
    }
...
bool ModeAuto::verify_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    ...
    //ok, so we reached destination, if _flags.do_cmd_all_done then do not continue.
 // if(copter.wp_nav->is_waiting_for_do_cmds()){
  //      return false; 
  //  }
  
   if(cmd.content.delay.seconds <= 0){
     copter.wp_nav->_flags.waypoint_completed = true; 
   }
    ...
    if(copter.wp_nav->waypoint_completed()){
        ...
        if (loiter_time == 0) {
            ...
            if (loiter_time_max > 0) {
                ...
                //no loiter time is set, so we should return true (which should send a mission_item_reached) 
                //return true; 
            }
        }
