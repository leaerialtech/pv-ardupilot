...
#include "AC_WPNav.h"
#include <AC_Avoidance/AP_OAPathPlanner.h>//needed for pv
    ...
    _attitude_control(attitude_control)
{
	...
	//PRECISIONVISION: 
	 _flags.reached_previous_wpt = false; 
     _flags.ignore_spray_heading = false; 
	 _flags.waypoint_completed = false; 
     // _flags.fast_waypoint = true;  //precisionVision assumes all waypoints are fast-waypoints UNLESS there is a delay specified, this seems to deviate from s-curve.
	///
...
bool AC_WPNav::set_wp_destination(const Vector3f& destination, bool terrain_alt)
{
	...
	//PRECISIONVISION//
	_flags.reached_previous_wpt = _flags.reached_destination;
    _flags.waypoint_completed = false; 
	///////////////////
...
bool AC_WPNav::advance_wp_target_along_track(float dt)
{
    ...
    curr_target_vel.z -= _pos_control.get_vel_offset_z_cms();
       ...
       //PRECISIONVISION/////////////////////////////////
        //this is a hack, probably should be running in the avoidance thread 
        AP_OAPathPlanner *oapath = AP_OAPathPlanner::get_singleton();
        AC_Avoid *_avoid = AP::ac_avoid();
        bool isAvoidingSomething = false; 
        Vector3f avoidance_vel_3d{curr_target_vel.x, curr_target_vel.y, 0.0f};

        //earlier versions had this defined, i'm just hard-coding this for now 
        if(_avoid && oapath && oapath->getType() == 10) // oapath->OA_PATHPLAN_PRECISIONVISION_STOP 
        {
            // Limit the velocity to prevent fence violations
            // TODO: We need to also limit the _desired_accel 
            _avoid->adjust_velocity(avoidance_vel_3d, _pos_control.get_pos_xy_p().kP(), _wp_accel_cmss, _pos_control.get_pos_z_p().kP(), _pos_control.get_max_accel_z_cmss(), dt);
            if(abs(curr_target_vel.x - avoidance_vel_3d.x) > .01 || abs(curr_target_vel.y - avoidance_vel_3d.y) > .01)
            {
                isAvoidingSomething = true; 
            }
            
            curr_target_vel = avoidance_vel_3d;
        }
    ...
    if (is_positive(_wp_desired_speed_xy_cms)) {
        ...
        shape_vel_accel((isAvoidingSomething ? 0.0 : vel_input), 0.0, _offset_vel, _offset_accel, -get_wp_acceleration(), get_wp_acceleration(),
    ...
    bool s_finished;
        ...
        if (!_this_leg_is_spline) {
            ...
            //PrecisionVision mod -- handle fastwaypoint vs non
            s_finished = _scurve_this_leg.advance_target_along_track(_scurve_prev_leg, _scurve_next_leg, 
            (_flags.fast_waypoint ?  _pv_fastwp_radius_cm : _wp_radius_cm) ,
             get_corner_acceleration(), _flags.fast_waypoint, _track_scalar_dt * vel_scaler_dt * dt, target_pos, target_vel, target_accel);
        } else {
            // splinetarget_vel
        ...
        if (is_positive(target_vel.length_squared())) {
            ...
            if(isAvoidingSomething){
                
                accel_offset = track_direction * _offset_accel * target_vel.length() / (-1*_wp_desired_speed_xy_cms);
    ...
    {
       // if (s_finished) //having this makes spray work, but screws up fast waypoint. has smoething to do with reached_destination i'm guessing. race condition with spray command?
       // {
        
			//PRECISIONVISION:  We modify stock code slightly such that WAYPOINT COMPLETED
			//DOES NOT OCCUR UNTIL ALL COMMANDS ARE FINISHED, THIS IS PRESENTLY
			//COMING EXTERNAL OF THE LIBRARY FROM THE AUTO FLIGHT MODE, NEEDS REFACTORING
				
		
            //Also, unlike stock firmware, we have different radius conditions in PV for 
			//fast and slow waypoints (this may not be as important in s-curve now, but 
			//we continue with the logic from earlier PV-specific flavors of ardu.
           
            const Vector3f dist_to_dest = curr_pos - _destination;
            if (dist_to_dest.length_squared() <= sq(_flags.fast_waypoint ?  _pv_fastwp_radius_cm : _wp_radius_cm )) 
            {
               _flags.reached_destination = true;
                if(_flags.fast_waypoint){
                      // printf("fastwp condition\n");
                }else{
                    //printf("slowwp condition\n");
                }
                 //printf("waypoint was reached\n");
            }else{
                //if we passed the apex of turn on a fast waypoint, we should consider it 'reached'
                if(_flags.fast_waypoint && s_finished){
                    _flags.reached_destination = true;
                }
            }
        //}    
...
bool AC_WPNav::set_spline_destination(const Vector3f& destination, bool terrain_alt, const Vector3f& next_destination, bool next_terrain_alt, bool next_is_spline)
{
    ...
    //bool orig = _flags.fast_waypoint;
	...
	//PRECISIONVISION//
    _flags.reached_previous_wpt = _flags.reached_destination;
    _flags.waypoint_completed = false; //todo: need to check implementaton for spline
	///////////////////
    return true;
}
