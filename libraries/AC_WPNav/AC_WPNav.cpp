...
bool AC_WPNav::set_wp_origin_and_destination(const Vector3f& origin, const Vector3f& destination, bool terrain_alt)
{
    ...
    //I'm going to try to repurpose this to prevent the mav from moving forward until all do_cmds have been completed.
    ...
    if(_flags.reached_destination){
            if(!AP_Mission::get_singleton()->is_do_commands_all_done())
            {
      ...
      //      }else{
     //           _flags.do_cmds_before_next_nav = false; 
     //       }
...
bool AC_WPNav::advance_wp_target_along_track(float dt)
{
    ...
    if (speed_along_track < -linear_velocity) {
        ...
        /*my attempt at limiting, but doesn't seem to execte here... 
          however, changing the constant WPNAV_YAW_DIST_MIN seems to have improved the turning of the aircraft in simulation        
         float targetYawRad  =  radians(get_bearing_cd(_origin, _destination) * 0.01f);
         float currentEstimatedYaw = _ahrs.yaw;

         float yawDiff = abs(currentEstimatedYaw - targetYawRad);
         if(yawDiff > 360){
              yawDiff = yawDiff - 360; 
         }
         if(yawDiff > 10){
            _limited_speed_xy_cms = .01;
            printf("limiting speeed\n");
         }else
         {
             */
    ...
    if( !_flags.reached_destination ) {
        if( _track_desired >= _track_length ) {
           // // previously "fast" waypoints are considered "completed" once the intermediate point reaches the destination
            //if (_flags.fast_waypoint) {
...
bool AC_WPNav::set_spline_origin_and_destination(const Vector3f& origin, const Vector3f& destination, bool terrain_alt, bool stopped_at_start, spline_segment_end_type seg_end_type, const Vector3f& next_destination)
{
    ...
    bool prev_segment_exists = (_flags.waypoint_completed && ((AP_HAL::millis() - _wp_last_update) < 1000));
...
bool AC_WPNav::advance_spline_target_along_track(float dt)
{
    if (!_flags.waypoint_completed) {
        ...
        if (is_zero(target_vel_length)) {
            _flags.waypoint_completed = true; //todo: need to check implementaton for spline
            _flags.reached_destination = true;
            return true;
        }
