...
bool AC_WPNav_OA::update_wpnav()
{
    ...
    if ((oa_ptr != nullptr) && AP::ahrs().get_location(current_loc)) {
        ...
        case AP_OAPathPlanner::OA_NOT_REQUIRED:
            ...
            if (!dest_to_next_dest_clear && (oa_ptr && oa_ptr->getType() != 10)) { //PRECISIONVISION - check its not our fake oa_type
                force_stop_at_next_wp();
            }
            break;

        case AP_OAPathPlanner::OA_PROCESSING:
        //PRECISIONVISION:  our 'type 10' is not a real obstacle avoidance, so just ignore and allow fast waypoint
            if (oa_ptr->get_options() & AP_OAPathPlanner::OA_OPTION_FAST_WAYPOINTS || oa_ptr->getType() == 10) {
