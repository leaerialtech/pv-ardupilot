...
void AP_Logger::Init(const struct LogStructure *structures, uint8_t num_types)
{
   ...
   //gcs().send_text(MAV_SEVERITY_INFO, "Init logging");
    ...
    if (_params.backend_types & uint8_t(Backend_Type::FILESYSTEM)) {
        ...
        //I think here is where we should do a filesystem check for disk-space,
        //and attempt to clear out the oldest file if we are at a certain level
