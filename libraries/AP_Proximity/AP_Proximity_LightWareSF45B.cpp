    ...
    case MessageID::DISTANCE_DATA_CM: {
        ...
        if (sector != _sector) {
            if (_sector != UINT8_MAX) {
                ...
                //smooth this out with our filter 

                const uint32_t now_ms = AP_HAL::millis();
                const uint32_t dt = now_ms - _last_update_ms[_sector];
                _last_update_ms[_sector] = now_ms;
                if (dt < PROXIMITY_FILT_RESET_TIME) {
                        _filtered_distance[_sector].set_cutoff_frequency(frontend.get_filter_freq());
                        _filtered_distance[_sector].apply(_sector_distance, dt* 0.001f);
                } else {
                    // reset filter since last distance was passed a long time back
                    _filtered_distance[_sector].reset(_sector_distance);
                }
