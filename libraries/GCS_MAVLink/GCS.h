...
#define MAX_MISSION_ITEM_REACHED_QUEUE (20)
...
public:
 ...
 static uint32_t last_radio_status_remrssi_ms() {
    ...
    // mission item index (stack in case we have many queued) to be sent on queued msg, delayed or not
    uint16_t mission_item_reached_index[MAX_MISSION_ITEM_REACHED_QUEUE] = {AP_MISSION_CMD_INDEX_NONE};
    uint16_t numReachedQueued = 0; //the "idx" for the array above 
...
protected:
    ...
    MAV_RESULT handle_command_user1(const mavlink_command_long_t &packet);
    MAV_RESULT handle_command_user2(const mavlink_command_long_t &packet);
    MAV_RESULT handle_command_user3(const mavlink_command_long_t &packet); 
...
public:
    ...
    void send_named_float(const char *name, float value) const;
    void send_spray_status(bool sprayerState);
