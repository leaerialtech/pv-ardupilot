...
//so, since we have ardupilot_mega as the dialect, I will repurpose the gopro command for precisionvision.
    ...
    uint8_t on = 1;
    uint8_t off = 0;
    float sw_m = sprayer->getSwathWidth();
                                 ...
                                 const mavlink_message_t &msg)
{
    ...
    int16_t target_systemc = -1;
    int16_t target_componentc = -1;
    routing.get_targets(msg, target_systemc, target_componentc);
...
void GCS::send_mission_item_reached_message(uint16_t mission_index)
{
    ...
    for (uint8_t i=0; i<num_gcs(); i++) {    
        chan(i)->mission_item_reached_index[chan(i)->numReachedQueued] = mission_index;
...
MAV_RESULT GCS_MAVLINK::handle_command_user2(const mavlink_command_long_t &packet) //, int16_t origin_sysid, int16_t origin_compid)
{    //return ;
...
MAV_RESULT GCS_MAVLINK::handle_command_long_packet(const mavlink_command_long_t &packet)
{
    ...
    switch (packet.command) {
        ...
        case MAV_CMD_USER_1:
            result = handle_command_user1(packet);//, origin_sysid, origin_compid);
        break;

        case MAV_CMD_USER_2:
            result = handle_command_user2(packet);//, origin_sysid, origin_compid);
        break; 
        case MAV_CMD_USER_3:
            result = handle_command_user3(packet);
            break; 
...
bool GCS_MAVLINK::try_send_mission_message(const enum ap_message id)
{
    ...
    case MSG_MISSION_ITEM_REACHED:
        ...
        //this is buggy, because if multiple_mission_item_Reached are queues, I think it drops because the index is overwritten 
        for(int q=numReachedQueued; q > 0; q--)
        {
            printf("numReachedQueued %d val %d\n", numReachedQueued, mission_item_reached_index[q-1]);
            mavlink_msg_mission_item_reached_send(chan, mission_item_reached_index[q-1]);
            mission_item_reached_index[q-1] = AP_MISSION_CMD_INDEX_NONE;
        }
        numReachedQueued=0;
        ret = true;
        break;
