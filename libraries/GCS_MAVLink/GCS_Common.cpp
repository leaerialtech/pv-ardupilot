...
//PRECISIONVISION
#include <PrecisionVisionLib/PVResumePointCreator.h>
#include <PrecisionVisionLib/PVTankSensorManager.h>
using namespace PrecisionVision;

//PV-TODO: fix me
//Needed by PrecisionVision -- for now, I guess 
...
{
        PVTankSensorManager* pvtank = PVTankSensorManager::get_singleton();

        TankSensorState tankState = pvtank->get_tank_sensor_status();
...
//so, since we have ardupilot_mega as the dialect, I will repurpose the gopro command for precisionvision.

    uint8_t on = 1;
    uint8_t off = 0;
    float sw_m = sprayer->get_swath_width();
    
    float rounded =  roundf( sw_m * 100.0 ) / 100.0;
    float whole = floorf(rounded);
    float decimal = (rounded - whole) * 100;

    uint8_t w = static_cast<uint8_t>(whole);
    uint8_t d = static_cast<uint8_t>(decimal);

    //we send down the swath width using two uint8_t's, one for the whole and the other for the decimal 
    uint8_t val[4] = {(sprayer->spraying() > 0 ? on : off), 0,  w, d};
...
void GCS_MAVLINK::handle_mission_set_current(AP_Mission &mission, const mavlink_message_t &msg)
{
    ...
    //PrecisionVision: 
    AC_Sprayer *sprayer = AC_Sprayer::get_singleton();
    sprayer->run(false,false); //disable the sprayer if it is running, it will be re-enabled when it hits the needed waypoint.
	
    //we need to notify auto mode, may need to refactor this as we learn this system's architecture better
    copter.wp_nav->resetReachedPreviousWaypoint();
	///////////////////
...
void GCS::service_pvresumepoint(){
 ...
 // create bitmask of what mavlink ports we should send this text to.
  ...
  if (_pv_resumepoint_queue.is_empty()) 
  {
        // nothing to do
        return;
  }
...
void GCS_MAVLINK::handle_named_value(const mavlink_message_t &msg) const
{
    ...
    char nameString[11] {};
    strncpy(nameString, p.name, sizeof(nameString)-1);

     
 
    if(strcmp(nameString, "WEIGHT") == 0){
        //forward on for demo
        gcs().send_named_float("WEIGHT", p.value);
         //gcs().send_text(MAV_SEVERITY_CRITICAL, "Weight is %f", p.value);
...
#if false //precisionvision removed landing gear for now AP_LANDINGGEAR_ENABLED
/*
...
MAV_RESULT GCS_MAVLINK::handle_command_user1(const mavlink_command_int_t &packet) //, int16_t origin_sysid, int16_t origin_compid)
{
     
    AC_Sprayer *sprayer = AC_Sprayer::get_singleton();

    //this is a little confusing, not sure if a bug?? because the packet parameters are numbered differently...

    //param 1 I believe was used early on to indicate if it is a 'manual' spray command instead of one put in auto-mode
    //param 2 is sprayer desired status (spray on/off)
    //param 3 indicates if we should use heading check or not

        
    bool ignoreHeadingCheck = packet.param3 > 0; 
   if(packet.param2 > 0){
...
MAV_RESULT GCS_MAVLINK::handle_command_user2(const mavlink_command_int_t &packet) //, int16_t origin_sysid, int16_t origin_compid)
{    //return ;
    
    return MAV_RESULT_ACCEPTED;
}

MAV_RESULT GCS_MAVLINK::handle_command_user3(const mavlink_command_int_t &packet)
{

    if(copter.get_mode() != (int)Mode::Number::BRAKE){
...
MAV_RESULT GCS_MAVLINK::handle_command_int_packet(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
	...
	//PrecisionVision//
	   case MAV_CMD_USER_1:
            return handle_command_user1(packet);//, origin_sysid, origin_compid);
        break;

        case MAV_CMD_USER_2:
            return handle_command_user2(packet);//, origin_sysid, origin_compid);fhandle_command_user3
        break; 
        case MAV_CMD_USER_3:
            return handle_command_user3(packet);
            break; 
...
#if false //AP_LANDINGGEAR_ENABLED
