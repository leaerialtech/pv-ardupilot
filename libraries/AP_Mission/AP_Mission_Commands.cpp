...
//this is PV's custom spray command 
bool AP_Mission::start_command_user_1(const AP_Mission::Mission_Command &cmd)
{
    switch(cmd.id){
          case MAV_CMD_USER_1:
			...
			//if param3 is > 0, we want to spray without heading-checks involved (path) 
           sprayer->run(cmd.p1 > 0, cmd.content.user1.ignoreHeadingChecks > 0);
